...
package com.akjava.lib.common.repackaged.org.apache.xmlgraphics.util;
...
public class DefaultEdgeDirectory implements EdgeDirectory {
    ...
    private Map<Vertex,Map<Vertex,Edge>> edges = new java.util.HashMap<Vertex,Map<Vertex,Edge>>();
    ...
    public void addEdge(Edge edge) {
        Map<Vertex,Edge> directEdges = edges.get(edge.getStart());
        if (directEdges == null) {
    ...
    public int getPenalty(Vertex start, Vertex end) {
        Map<Vertex,Edge> edgeMap = edges.get(start);
        if (edgeMap != null) {
	...
	public Iterator<Vertex> getDestinations(Vertex origin) {
        Map<Vertex,Edge> directRoutes = edges.get(origin);
        if (directRoutes != null) {
            Iterator<Vertex> iter = directRoutes.keySet().iterator();
            return iter;
        }
