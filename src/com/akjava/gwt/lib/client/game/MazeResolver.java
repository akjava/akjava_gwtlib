...
public class MazeResolver {
	...
	public MazeResolver(int[][] mapDataYX,List<Integer> wallNumbers){
		this.map=mapDataYX;
...
public static DefaultEdgeDirectory mapToDirectory(HashMap<String,PtVertex> pts,int[][] map,List<Integer> wallNumbers){
		...
		for(int y=0;y<map.length;y++){
			for(int x=0;x<map[y].length;x++){
				...
				//each eay
				directory.addEdge(new PtEdge(pt, pt2));
				directory.addEdge(new PtEdge(pt2, pt));
